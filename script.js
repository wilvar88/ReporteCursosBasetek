            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    formatter: function(value, context) {
                        if (!context.chart.$animStartTime) {
                            context.chart.$animStartTime = Date.now();
                        }
                        let p = Math.min((Date.now() - context.chart.$animStartTime) / 2500, 1);
                        let easeOut = 1 - Math.pow(1 - p, 4);
                        return (easeOut * value).toFixed(1);
                    }
                }
            },
            scales: {
