...
async function fetchDashboardData() {
    try {
        ...
        document.getElementById('kpi-avg-rating').innerText = dashboardData.kpis.average_rating ? dashboardData.kpis.average_rating.toFixed(1) : '-.--';
        document.getElementById('kpi-rating-count').innerText = dashboardData.kpis.ratings_count || '0';
        document.getElementById('kpi-rating-participation').innerText = `${(dashboardData.kpis.rating_participation || 0).toFixed(1)}%`;
...
function updateKPIs(kpis) {
    document.getElementById('kpi-participation').innerText = `${kpis.participation_rate.toFixed(1)}%`;
    document.getElementById('kpi-approval').innerText = `${kpis.approval_rate.toFixed(1)}%`;
    document.getElementById('kpi-courses').innerText = kpis.total_courses;
    document.getElementById('kpi-attendees').innerText = kpis.total_attendees;
    
    const partCountEl = document.getElementById('kpi-part-count');
    if (partCountEl && kpis.global_started !== undefined) partCountEl.innerText = kpis.global_started;
    
    const apprCountEl = document.getElementById('kpi-appr-count');
    if (apprCountEl && kpis.global_approved !== undefined) apprCountEl.innerText = kpis.global_approved;
}
...
function renderTable(courses) {
    ...
    courses.forEach(c => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 text-center text-gray-300 font-medium">${c.enrolled}</td>
            <td class="px-6 py-4">
                <div class="flex flex-col">
                    <span class="text-right font-bold text-blue-400">${c.participation.toFixed(1)}%</span>
            ...
            <td class="px-6 py-4">
                <div class="flex flex-col">
                    <span class="text-right font-bold text-green-400">${c.approval.toFixed(1)}%</span>
                    <div class="table-progress-bar">
    ...
    setTimeout(() => {
        const fills = document.querySelectorAll('.table-progress-fill');
        fills.forEach(fill => {
...
function renderAreasTable(areas) {
    ...
    areas.forEach((a, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 text-center text-gray-300 font-medium">${a.enrolled}</td>
            <td class="px-6 py-4">
                <div class="flex flex-col">
                    <span class="text-right font-bold text-blue-400">${a.participation.toFixed(1)}%</span>
                    <div class="table-progress-bar">
            ...
            <td class="px-6 py-4">
                <div class="flex flex-col">
                    <span class="text-right font-bold text-green-400">${a.approval.toFixed(1)}%</span>
                    <div class="table-progress-bar">
